...
export default {
  ...
  methods: {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.$confirm("是否要提交修改?", "提示", {
            ...
            // })
            this.$http({
              url: this.$http.adornUrl("/orderConfig/updateOrderConfig/1"),
              method: "post",
    ...
    getDetail() {
      ...
      // });
      this.$http({
        url: this.$http.adornUrl("/orderConfig/1"),
        method: "get",
      }).then(({ data }) => {
        if (data.status === 1) {
          this.orderSetting = data.data;
          this.$message({
            type: "success",
            message: "设置成功!"
          });
