...
import LogisticsDialog from "./component/logisticsDialog";
...
export default {
  ...
  methods: {
    ...
    handleCloseOrderConfirm() {
      ...
      // });
      this.$http({
        url: this.$http.adornUrl("/order/close"),
        method: "post",
        params: params
      }).then(({ data }) => {
        if (data.status === 1) {
    ...
    getList() {
      ...
      this.$http({
        url: this.$http.adornUrl("/order/listOrder"),
        method: "post",
    ...
    covertOrder(order) {
      let address = order.receiverDetailAddress;
      let listItem = {
        orderId: order.id,
