        ...
        <el-table-column label="是否推荐" width="200" align="center">
          <template slot-scope="scope">
            <el-switch
              ...
              :active-value="1"
              :inactive-value="0"
              v-model="scope.row.peopleRecommendStatus"
            ></el-switch>
          </template>
        </el-table-column>
        <el-table-column label="排序" width="160" align="center">
          <template slot-scope="scope">{{scope.row.peopleSort}}</template>
        </el-table-column>
        <el-table-column label="状态" width="160" align="center">
          <template slot-scope="scope">{{scope.row.peopleRecommendStatus | formatRecommendStatus}}</template>
        </el-table-column>
    ...
    <el-dialog title="选择商品" :visible.sync="selectDialogVisible" width="50%">
      ...
      <el-table :data="dialogData.list" @selection-change="handleDialogSelectionChange" border>
        ...
        <el-table-column label="货号" width="160" align="center">
          <template slot-scope="scope">NO.{{scope.row.sn}}</template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleRecommendStatusStatusChange(index, row) {
      this.updateRecommendStatusStatus(
        row.productId,
        row.peopleRecommendStatus
      );
    },
    ...
    handleSelectDialogConfirm() {
      ...
      this.$confirm("使用要进行添加操作?", "提示", {
        ...
        // });
        this.$http({
          url: this.$http.adornUrl("/smsPeopleRecommend/createPeople"),
          method: "post",
          data: { peopleRecommend: selectProducts }
        }).then(({ data }) => {
          if (data.status === 1) {
            this.selectDialogVisible = false;
            this.dialogData.multipleSelection = [];
    ...
    handleEditSort(index, row) {
      ...
      this.sortDialogData.id = row.productId;
    },
    handleUpdateSort() {
      this.$confirm("是否要修改排序?", "提示", {
        ...
        this.$http({
          url: this.$http.adornUrl(
            "/smsPeopleRecommend/sortPeople/" + this.sortDialogData.id
          ),
          method: "post",
          data: this.sortDialogData
        }).then(({ data }) => {
          if (data.status === 1) {
            this.sortDialogVisible = false;
            this.getList();
    ...
    getList() {
      ...
      // })
      this.$http({
        url: this.$http.adornUrl("/smsPeopleRecommend/listPeople"),
        method: "post",
    ...
    deleteProduct(ids) {
      this.$confirm("是否要删除该推荐?", "提示", {
        ...
        // });
        this.$http({
          url: this.$http.adornUrl("/smsPeopleRecommend/deletePeople"),
          method: "post",
          params: params
        }).then(({ data }) => {
          if (data.status === 1) {
    ...
    getDialogList() {
      ...
      this.$http({
        ...
        method: "post",
        data: this.$http.adornData(this.dialogData.listQuery)
      }).then(({ data }) => {
        if (data.status === 1) {
          this.dialogData.list = data.data.list;
          this.dialogData.total = data.data.total;
        }
      });
    }
  }
