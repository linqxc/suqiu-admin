...
export default {
  ...
  methods: {
    ...
    handleRecommendStatusStatusChange(index, row) {
      this.updateRecommendStatusStatus(row.brandId, row.recommendStatus);
    },
    handleDelete(index, row) {
      this.deleteBrand(row.brandId);
    },
    handleBatchOperate() {
      ...
      for (let i = 0; i < this.multipleSelection.length; i++) {
        ids.push(this.multipleSelection[i].brandId);
      }
    ...
    handleSelectDialogConfirm() {
      ...
      this.$confirm("使用要进行添加操作?", "提示", {
        ...
        // });
        this.$http({
          url: this.$http.adornUrl("/smsBrandRecommend/createBrand"),
          method: "post",
          data: { brandRecommend: selectBrands }
        }).then(({ data }) => {
          if (data.status === 1) {
            this.selectDialogVisible = false;
            this.dialogData.multipleSelection = [];
    ...
    handleEditSort(index, row) {
      ...
      this.sortDialogData.id = row.brandId;
    },
    handleUpdateSort() {
      this.$confirm("是否要修改排序?", "提示", {
        ...
        this.$http({
          url: this.$http.adornUrl(
            "/smsBrandRecommend/sortBrand/" + this.sortDialogData.id
          ),
          method: "post",
    ...
    getList() {
      ...
      // })
      this.$http({
        url: this.$http.adornUrl("/smsBrandRecommend/listBrand"),
        method: "post",
    ...
    updateRecommendStatusStatus(ids, status) {
      this.$confirm("是否要修改推荐状态?", "提示", {
        ...
        .then(() => {
          ...
          // });
          this.$http({
            url: this.$http.adornUrl("/smsBrandRecommend/isBrandRecommend"),
            method: "post",
            params: params
          }).then(({ data }) => {
            if (data.status === 1) {
    ...
    deleteBrand(ids) {
      this.$confirm("是否要删除该推荐?", "提示", {
        ...
        // });
        this.$http({
          url: this.$http.adornUrl("/smsBrandRecommend/deleteBrand"),
          method: "post",
          params: params
        }).then(({ data }) => {
          if (data.status === 1) {
